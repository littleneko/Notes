可能对很多 C++ 程序员而言，Type Traits 并不陌生，它被大量应用在模板元编程中。从字面上理解，Type Traits 就是 “类型的特征” 的意思。在 C++ 元编程中，程序员不少时候都需要了解一些类型的特征信息，并根据这些类型信息选择应有的操作。Type Traits 有助于编写通用、可复用的代码。
