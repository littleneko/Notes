...
1. **权限确定**：在**连接器**里处理完登录请求后就查询出来确定，以后不变
1. **长连接**：长连接使用临时内存直到连接断开时才释放，MySQL 5.7以后可以通过 `mysql_reset_connection` 初始化
1. **权限验证**：执行器在执行之前会判断
...
<img src="https://littleneko.oss-cn-beijing.aliyuncs.com/img/1584288534049-262d25e8-4663-4f54-8d58-acad70498b3c.png" alt="image.png" style="zoom:50%;" />
...
1. **事物隔离的实现**：可重复读 -> MVCC，undo
1. **undo删除的时机**：当系统里么有比这个回滚日志更早的 read-view 的时候
...
1. **InnoDB索引模型**：主键索引 B+ 树，每一个索引 B+ 树
...
2. **最左前缀原则**：B+ 树这种索引结构，可以利用索引的“最左前缀”，来定位记录。
    ...
    <img src="https://littleneko.oss-cn-beijing.aliyuncs.com/img/1584546639627-eef36114-dfcd-4b2e-ab8c-8f2a2dc41813.png" alt="image.png" style="zoom:50%;" />
...
Q1: mysql 5.6不是支持 online ddl 了吗？也就是对表操作增加字段等功能，实际上不会阻塞读写
